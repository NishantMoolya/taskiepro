...
const Homepage = () => {
    return (
        <div className='flex flex-col gap-3'>
            ...
            <div className='flex flex-col sm:flex-row flex-wrap sm:items-center px-2 sm:px-0'>
            <ProgressBar />
            {/* <div className='bg-violet-500 p-3 flex flex-col items-center justify-center text-white rounded-xl'>
            ...
            <div className='flex flex-col gap-4'>
                <div className='flex flex-col'>
                    <p className='bg-white py-1 px-3 text-lg font-semibold capitalize sm:border rounded-t-xl text-violet-500'>Todos(4)</p>
                    <div className='flex flex-col sm:flex-row bg-slate-100 py-2 sm:rounded-b-xl'>
                        <TaskSubContainer list={taskList.filter(val => val.completed === false)} />
                        <TaskSubContainer list={taskList.filter(val => val.completed === true)} />
                    </div>
                </div>
                <div className='flex flex-col'>
                    <p className='bg-white py-1 px-3 text-lg font-semibold capitalize sm:border rounded-t-xl text-violet-500'>Reminders(2)</p>
                    <div className='flex flex-col gap-2 bg-slate-100 py-4 px-2 sm:rounded-b-xl'>
                        {
                            taskList.map((task, ind) => <ReminderItem key={ind} />)
                        }
                    </div>
                </div>
            </div>
