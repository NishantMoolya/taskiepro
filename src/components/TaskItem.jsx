...
import { markCompleted } from '../redux/reducers/taskReducer';
  ...
  return (
    <div className={`bg-white flex gap-2 px-2 py-2 pt-1 rounded shadow border-t-4 ${task.completed ? 'border-green-500' : 'border-sky-400'}`}>
      {task.completed ? <input type="checkbox" checked={true} className='appearance-none w-4 h-4 border-2 border-green-500 rounded-full mt-1 relative peer checked:ring-1 checked:ring-green-500 checked:bg-green-500 checked:text-white shrink-0 hover:cursor-pointer' />
        : <input type="checkbox" onClick={() => taskDispatch(markCompleted(task._id))} className='appearance-none w-4 h-4 border-2 border-green-500 rounded-full mt-1 relative peer checked:ring-1 checked:ring-green-500 checked:bg-green-500 checked:text-white shrink-0 hover:cursor-pointer' />}
      <svg
      ...
      {!task.completed && <button onClick={() => taskDispatch(removeTask(task._id,"todo"))} className='text-red-500 mt-1 flex self-start'>
